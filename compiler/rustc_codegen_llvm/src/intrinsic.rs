        ...
        let llval = match name {
            ...
            sym::raw_eq => {
                ...
                let use_integer_compare = match layout.abi() {
                    ...
                    Uninhabited | Vector { .. } | WasmHeapRef { .. } => false,
                    Aggregate { .. } => {
            ...
            sym::wasm_global_get => {
                // FIXME: Handle types other than `externref`.
                // FIXME: LLVM does not currently support explicit
                // initialization of Wasm globals. Instead, in LLVM IR, they are
                // initialized to `undef`, which results in `null` when compiled
                // to Wasm. The globals generated by us are may have `internal`
                // linkeage, which means that if LLVM does not find a write to
                // the global in the current module, it would optimize away this
                // `load`, returning `undef` (instead of `null`). The `voltile`
                // prevents this.
                self.volatile_load(self.type_wasm_externref(), args[0].immediate())
            }

            sym::wasm_global_set => {
                // FIXME: Handle types other than `externref`.
                self.store(args[1].immediate(), args[0].immediate(), Align::ONE)
            }
...
fn codegen_msvc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let type_info_vtable = bx.declare_global("??_7type_info@@6B@", bx.type_ptr(), 0);
        let type_name = bx.const_bytes(b"rust_panic\0");
        let type_info =
            bx.const_struct(&[type_info_vtable, bx.const_null(bx.type_ptr()), type_name], false);
        let tydesc = bx.declare_global("__rust_panic_type_info", bx.val_ty(type_info), 0);
        unsafe {
